...
import { ParticipantListPage as ShowCaseList } from 'pages/participants';

function DashboardPage() {
  const { session } = useSession();

  const conferenceStat = useSWR<PaginatedInterface<ConferenceInterface>>(
    () => `/conferences`,
    () => getConferences({ limit: 1, offset: 0 }),
  );

  const participantStat = useSWR<PaginatedInterface<ParticipantInterface>>(
    () => `/participants`,
    () => getParticipants({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Conferences',
      result: conferenceStat?.data?.totalCount,
      isLoading: conferenceStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Participants',
      result: participantStat?.data?.totalCount,
      isLoading: participantStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`participants`} passHref legacyBehavior>
                  <Button
