...
import { getConferenceById, updateConferenceById } from 'apiSdk/conferences';
...
function ConferenceEditPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<ConferenceInterface>(
    () => (id ? `/conferences/${id}` : null),
  ...
  const handleSubmit = async (values: ConferenceInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateConferenceById(id, values);
      mutate(updated);
      resetForm();
